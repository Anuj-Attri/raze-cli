...
def build_graph(files: List[FileMeta],
                ...
                age_clusters: Dict[str, List[str]]) -> Dict[str, Any]:
    nodes: List[Node] = []
    edges: List[Edge] = []

    # Root
    root_id = "root"
    nodes.append(Node(id=root_id, kind="root", label="Scan Root", meta={}, reasoning={"rationale": "Root of scan.", "confidence": 1.0}))

    # Duplicate clusters
    for h, ids in dup_clusters.items():
        nid = f"dup:{h[:8]}"
        reasoning = suggest_node_reasoning("duplicate_cluster", {"file_ids": ids})
        nodes.append(Node(id=nid, kind="duplicate_cluster", label=f"Duplicates {h[:8]}", meta={"hash": h, "file_ids": ids}, reasoning=reasoning))
    ...
    for t, ids in type_clusters.items():
        ...
        nodes.append(Node(id=nid, kind="type_cluster", label=f"Type: {t}", meta={"file_ids": ids}, reasoning=reasoning))
        edges.append(Edge(id=f"e-{root_id}-{nid}", source=root_id, target=nid, kind="contains"))
        for fid in ids[:50]:  # don't explode edges; sample
    ...
    for label, ids in age_clusters.items():
        ...
        nodes.append(Node(id=nid, kind="age_bucket", label=f"Age: {label}", meta={"file_ids": ids}, reasoning=reasoning))
        edges.append(Edge(id=f"e-{root_id}-{nid}", source=root_id, target=nid, kind="contains"))
        for fid in ids[:50]:
            edges.append(Edge(id=f"e-{nid}-{fid}", source=nid, target=fid, kind="contains_sample"))
    ...
    for f in files[:200]:
        nodes.append(Node(id=f.id, kind="file", label=f.path, meta={"size": f.size, "mtime": f.mtime, "type": f.type, "hash": f.hash}, reasoning={"rationale":"Raw file node", "confidence":1.0}))
